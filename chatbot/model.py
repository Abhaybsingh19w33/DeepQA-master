    ...
    def buildNetwork(self):
        ...
        def create_rnn_cell():
            ...
            if not self.args.test:  # TODO: Should use a placeholder instead
                ...
                encoDecoCell = tf.compat.v1.nn.rnn_cell.DropoutWrapper(
                    encoDecoCell,
                    input_keep_prob=1.0,
        ...
        with tf.compat.v1.name_scope('placeholder_encoder'):
            self.encoderInputs  = [tf.compat.v1.placeholder(tf.int32,   [None, ]) for _ in range(self.args.maxLengthEnco)]  # Batch size * sequence length * input dim

        with tf.compat.v1.name_scope('placeholder_decoder'):
            self.decoderInputs  = [tf.compat.v1.placeholder(tf.int32,   [None, ], name='inputs') for _ in range(self.args.maxLengthDeco)]  # Same sentence length for input and output (Right ?)
            self.decoderTargets = [tf.compat.v1.placeholder(tf.int32,   [None, ], name='targets') for _ in range(self.args.maxLengthDeco)]
            self.decoderWeights = [tf.compat.v1.placeholder(tf.float32, [None, ], name='weights') for _ in range(self.args.maxLengthDeco)]
